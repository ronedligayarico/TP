...
ResultSet rsSelect1 = null;
	...
	String studID1 = request.getParameter("studID1");
	...
	try
	{
		...
		else if(password1.equals(""))
		{
			...
			{
				querySelect = "SELECT * FROM `newproject`.`users` WHERE `user_number` = '"+studID1+"' AND `markasdeleted` = 0 AND `user_type`='1'";
				rsSelect = stmtAdminLogin1.executeQuery(querySelect);
				if(rsSelect.next())
				{
						querySelect = "SELECT `user_id` FROM `newproject`.`users` WHERE `user_number` = '"+studID1+"' AND `user_pass` = MD5('"+password1+"') AND `markasdeleted` = 0 AND `user_type`='1'";
